...
class AnalyticsApiController {
    ...
    public AnalyticsApiController(TradeRepository tradeRepo) {
    ...
    public Map<String, Object> metrics(
        ...
        if ("today".equalsIgnoreCase(period)) {
            ...
            if (to == null)
                to = now;
            if (from == null)
                from = to.minus(Duration.ofHours(2));
        }
        ...
        for (Trade t : trades) {
            ...
            perAsset.computeIfAbsent(sym, k -> new ArrayList<>())
                    .add(Map.of("ts", t.getTs().toString(), "value", cm.get(sym)));
        }

        Map<String, Object> res = new LinkedHashMap<>();
        res.put("from", from.toString());
        res.put("to", to.toString());
        res.put("count", trades.size());
        res.put("cumulativePnL", cumSeries);
        res.put("perAssetCumulative", perAsset);
        return res;
    }
}
