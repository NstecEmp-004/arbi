...
public class StateTableApiController {
    private final MarketRepository marketRepo;
    private final AssetRepository assetRepo;
    private final TradeRepository tradeRepo;
    private final MarketState state;
    // public StateTableApiController(MarketRepository marketRepo, AssetRepository assetRepo,
    // TradeRepository tradeRepo, MarketState state) {
    // this.marketRepo = marketRepo; this.assetRepo = assetRepo; this.tradeRepo =
    // tradeRepo; this.state = state;
    // }
    ...
    public Map<String, Object> state() {
        ...
        for (Market m : markets) {
            ...
            for (Asset a : assets)
                row.put(a.getSymbol(), mp.get(a.getId()) != null ? mp.get(a.getId()).price() : null);
            table.add(row);
        }
        ...
        for (Trade t : trades) {
            ...
            m.put("qty", t.getQuantity());
            m.put("buyPrice", t.getBuyPrice());
            m.put("sellPrice", t.getSellPrice());
            m.put("profit", t.getProfit());
            m.put("ts", t.getTs().toString());
            trs.add(m);
        }
        res.put("trades", trs);
        return res;
