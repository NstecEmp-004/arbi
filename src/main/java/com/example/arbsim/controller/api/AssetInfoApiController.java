...
public class AssetInfoApiController {
    ...
    // public AssetInfoApiController(AssetRepository assetRepo, MarketRepository
    // marketRepo,
    ...
    public Map<String, Object> summary(@PathVariable String symbol) {
        ...
        if (curCount > 0)
            curAvg = curAvg.divide(BigDecimal.valueOf(curCount), 4, RoundingMode.HALF_UP);
    ...
    public Map<String, Object> addComment(@PathVariable String symbol, @RequestParam("text") String text) {
        ...
        if (text.isEmpty())
            return Map.of("ok", false, "message", "empty");
